...
Now move the existing API server certificate and key out of the `/etc/kubernetes/pki` directory, because **if kubeadm sees that they already exist it won’t create new ones**!. 
```
